...
/// Compares HIT-Leiden against igraph Leiden baseline
...
fn write_exports(
    ...
    let mut csv = String::from(
        "batch_idx,edges_added,total_edges,nodes_in_graph,hit_leiden_time_ms,igraph_time_ms,speedup,hit_leiden_iterations,hit_modularity,igraph_modularity\n",
    );
    ...
    for (i, batch) in outcome.batches.iter().enumerate() {
        ...
        batches_json.push_str(&format!(
            "{{\"batch_idx\":{},\"edges_added\":{},\"total_edges\":{},\"nodes_in_graph\":{},\"hit_leiden_time_ms\":{:.6},\"igraph_time_ms\":{:.6},\"speedup\":{:.6},\"hit_leiden_iterations\":{},\"hit_modularity\":{:.6},\"igraph_modularity\":{:.6}}}",
            batch.batch_idx,
            batch.edges_added,
            batch.total_edges,
            batch.nodes_in_graph,
...
fn main() {
    ...
    match run_incremental(split.update_batches, batch_size, initial_edges) {
        Ok(outcome) => {
            ...
            println!(
                "{:<6} {:<12} {:<12} {:<12} {:<10}",
                "Batch", "Edges", "HIT (ms)", "igraph(ms)", "Speedup"
            );
            println!("{}", "-".repeat(50));
